...
def _load_crawler_auth_profiles() -> dict[str, dict[str, Any]]:
    """Load per-engine crawler auth profiles from settings manager."""
    return settings_manager.get_crawler_auth_profiles()
...
async def list_engines() -> dict[str, Any]:
    ...
    manager = CrawlerManager(config, auth_profiles=_load_crawler_auth_profiles())
    return {
...
async def download_papers(
    ...
    auth_profiles = _load_crawler_auth_profiles()
    downloader = PDFDownloader(ref_dir, crawler_auth=auth_profiles)
    ...
    async def _run_single_download(result: SearchResult, job_id: str):
        ...
        try:
            ...
            downloader = PDFDownloader(
                ref_dir,
                crawler_auth=_load_crawler_auth_profiles(),
            )
            async with downloader:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                if resp.status_code == 200:
                    ...
                    citation_authors = soup.find_all(
                    ...
                    if citation_authors:
                        authors = [
                            a["content"].strip()
                            for a in citation_authors
                            if a.get("content")
                        ]
                    elif dc_creators:
                        authors = [
                            a["content"].strip()
                            for a in dc_creators
                            if a.get("content")
                        ]
                    ...
                    return FetchMetadataResponse(
                        ...
                        authors=[source_label]
                        if source_label
                        else ["Third-party Website"],
                        year=None,
                        source=domain,
                        pdf_url=None,
                        is_academic=False,
