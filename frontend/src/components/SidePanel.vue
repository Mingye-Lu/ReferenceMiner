...
const submenuPosition = ref<{ top?: number; bottom?: number; left: number }>({
  bottom: 0,
  left: 0,
});
...
async function handleUploadComplete(entry: ManifestEntry) {
  ...
  if (projectId.value && entry.relPath) {
    // Use projectId.value instead of props.projectId
...
async function confirmDeleteFile() {
  ...
  try {
    ...
    // Remove from project
    await removeProjectFiles(projectId.value, [
      pendingDeleteFile.value.relPath,
    ]);
...
function handleClickOutside(event: MouseEvent) {
  ...
  if (
    showSettings.value &&
    !target.closest(".settings-popup-panel") &&
    !target.closest(".settings-trigger-btn")
  ) {
  ...
  <aside class="sidebar-shell">
    ...
    <div class="sidebar-content" v-if="activeTab === 'corpus'">
      ...
      <div class="sidebar-scrollable">
          ...
          <div v-if="batchMode && displayedFiles.length > 0" class="batch-toolbar">
            ...
            <div class="batch-actions">
              <button class="batch-action-pill" @click="selectAllForBatch">
                {{
                  batchSelected.size === displayedFiles.length
                    ? "Deselect"
                    : "All"
                }}
              </button>
        ...
        <div v-if="displayedFiles.length === 0" class="empty-msg">
...
.settings-popup-panel {
  ...
  border-radius: 12px;
  box-shadow:
    0 8px 24px var(--alpha-black-15),
    0 2px 8px var(--alpha-black-10);
  padding: 6px;
  z-index: 1000;
  min-width: 240px;
}
